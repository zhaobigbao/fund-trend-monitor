...
import { listFundStockTags } from "./stockTagService.mjs";
...
const SOURCE_DOMAIN_CAPABILITIES = {
  fund: ["fundSearch"],
  fundNav: ["fundNav"],
  fundHoldings: ["fundHoldings"],
  stockTags: ["stockTags"],
  sectorBoard: ["industryBoards", "conceptBoards", "boardIntraday"],
  researchReports: ["researchReports"]
};

export function listDataSources() {
  return {
    sources: enrichSources(listDataSourceRecords()),
    defaults: listDataSourceDefaultRecords()
  };
}

export function updateDataSourceDefaults(input = {}) {
  const allowedSources = new Set(enrichSources(listDataSourceRecords()).filter((source) => source.enabled).map((source) => source.id));
  for (const [domain, sourceId] of Object.entries(input.defaults || {})) {
    if (SOURCE_DOMAIN_LABELS[domain] && allowedSources.has(sourceId) && sourceSupportsDomain(sourceId, domain)) upsertDataSourceDefault(domain, sourceId);
  }
...
export function updateFundSourceBindings(code, input = {}) {
  ...
  for (const [domain, sourceId] of Object.entries(input.bindings || {})) {
    if (SOURCE_DOMAIN_LABELS[domain] && allowedSources.has(sourceId) && sourceSupportsDomain(sourceId, domain)) upsertFundSourceBinding(fund.code, domain, sourceId);
  }
...
export function buildSourceCoverage(code) {
  const fund = getFundRecord(code);
  const sources = enrichSources(listDataSourceRecords());
  ...
  const stockTags = listFundStockTags(fund.code);
  ...
  const domains = Object.keys(SOURCE_DOMAIN_LABELS).map((domain) => {
    ...
    return {
      ...
      status: coverageStatus(domain, { holdings, navSummary, stockTags }),
      detail: coverageDetail(domain, { holdings, navSummary, stockTags }),
      options: sources.map((candidate) => ({
        id: candidate.id,
        name: candidate.name,
        enabled: candidate.enabled,
        supported: candidate.supportedDomains.includes(domain),
...
function coverageStatus(domain, { holdings, navSummary }) {
  ...
  if (domain === "stockTags") return holdings.rows.length ? "ready" : "pending";
  if (domain === "sectorBoard") return holdings.rows.length ? "ready" : "needsHoldings";
  if (domain === "researchReports") return "partial";
  return "ready";
}

function coverageDetail(domain, { holdings, navSummary, stockTags }) {
  ...
  if (domain === "stockTags") return stockTags.rows.length ? `${stockTags.rows.length} 只持仓股标签` : "等待持仓股票标签";
...
function enrichSources(sources) {
  return sources.map((source) => ({
    ...source,
    supportedDomains: Object.keys(SOURCE_DOMAIN_LABELS).filter((domain) => sourceCanServeDomain(source, domain))
  }));
}

function sourceSupportsDomain(sourceId, domain) {
  const source = enrichSources(listDataSourceRecords()).find((item) => item.id === sourceId);
  return Boolean(source?.supportedDomains.includes(domain));
}

function sourceCanServeDomain(source, domain) {
  const requiredCapabilities = SOURCE_DOMAIN_CAPABILITIES[domain] || [];
  return requiredCapabilities.every((capability) => Boolean(source.capabilities?.[capability]));
}
