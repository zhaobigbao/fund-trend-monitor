...
真实数据源接入与数据源切换的专项设计维护在 `docs/DATA_SOURCE_STRATEGY.md`。涉及外部数据源、板块实时行情、源级能力矩阵和合规边界时，需要同步阅读该文档。
...
数据接入层需要支持多数据源 adapter，前端不得直接请求第三方数据源。每个 adapter 必须声明能力边界、健康状态和最近同步时间，服务层按用户选择或默认优先级取数。
...
- 不直接生成 AI 结论。
- 只消费标准化后的基金行情和净值数据。
- 不直接决定数据源优先级，数据源选择由数据接入层和用户配置共同决定。
...
- 维护股票所属行业、板块、概念、赛道
- 为持仓分析和研报聚合提供标签能力
- 维护外部概念板块与内部赛道名称的别名映射

边界：

- 标签变化需要保留更新时间和来源。
- 同一股票可以属于多个赛道，但应有主行业。
- 概念板块映射只能作为辅助分析，不能等同基金实时持仓。
...
- 提取研报摘要、来源、发布时间、观点倾向
- 为 AI 结论提供证据材料
- 输出研报匹配原因，例如命中个股、赛道或行业，以及对应持仓权重

边界：

- 正式产品中必须确认研报数据授权和使用合规性。
- 不在前端直接暴露未经处理的大段研报原文。
- 研报推荐必须能解释匹配来源，不做无依据的推荐排序。
...
- 管理默认数据源和基金级数据源绑定
- 展示数据源能力覆盖、健康状态和最近同步时间
- 支持按业务域切换来源，例如净值、持仓、板块、研报
- 为服务层提供统一 source selection
...
- 基金详情页按业务阅读顺序组织：实时监测和基础操作在前，持仓、AI 结论、研报等主分析模块纵向展开，避免把两个长内容模块并排放置。
- 预警、行业暴露、板块实时雷达等摘要模块可以并列展示，但同一行内应保持稳定高度；内容超出时优先在模块内部滚动，避免页面卡片高低参差。
- 长内容模块允许自然高度，短内容模块适合固定高度。后续新增公告、基金经理、估值分位、事件提醒时，应先判断信息密度，再决定纵向主流或摘要并列。
- 数据同步、数据源覆盖、同步记录属于操作与状态管理，不应占用主分析阅读流；基金详情页顶部保留统一入口，详细同步状态在弹窗中集中展示。
- 页面布局只调整信息层级和交互，不在前端重新推导持仓、预警、板块或 AI 结论。
...
- `/api/funds/:code/source-bindings`
- `/api/funds/:code/sector-radar`
- `/api/data-sources`
- `/api/data-sources/defaults`
- `/api/watchlists`
...
- `research_reports`：研报元数据和摘要，包含个股、赛道、行业匹配字段
- `ai_insights`：AI 分析结论快照，包含结构化 sections、兼容字段、生成时间、落库时间和内容签名
- `watchlists`：用户自选基金
- `alerts`：预警规则和触发记录
- `data_sources`：外部数据源配置、能力矩阵和健康状态
- `data_source_defaults`：业务域默认数据源
- `fund_source_bindings`：基金级数据源绑定
- `sector_aliases`：外部板块名称与内部行业/赛道名称映射
- `sector_realtime_quotes`：行业/概念板块实时行情快照
- `sector_intraday_points`：行业/概念板块分钟走势
- `sector_constituents`：行业/概念板块成份股
...
- `snapshotId`：结论落库后的快照 ID。
- `recordedAt`：结论快照落库时间。

`/api/funds/:code/insights` 返回最近的 AI 结论快照，用于页面历史回看和后续趋势复盘。
...
- 研报匹配应优先按持仓股票代码、赛道、行业依次聚合，输出结果需要带 `matchLabel` 和 `matchWeight`。
- AI 结论历史由 `ai_insights` 统一保存，前端不得用浏览器本地状态模拟历史记录。
- 数据源切换必须通过服务层读取 `data_sources` 与绑定关系，前端只提交选择结果。
- 板块实时走势必须保留板块来源、板块类型、更新时间和与基金持仓的匹配依据。
